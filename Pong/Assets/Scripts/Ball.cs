...
public class Ball : MonoBehaviour {
	...
	void OnCollisionEnter2D(Collision2D col){
		...
		if ((col.gameObject.name == "HorzWallbottom") || (col.gameObject.name == "HorzWallTop")) {

			// Call for SoundManager to play the wall sound
			SoundManager.Instance.PlayOneShot (SoundManager.Instance.wallBloop);
		}

		if ((col.gameObject.name == "VertWallleft") || (col.gameObject.name == "VertWallRitgh")) {
			...
			if (col.gameObject.name == "VertWallleft") {
				...
				increaseTextUIScore ("RigthTextUI");

			} else if (col.gameObject.name == "VertWallRitgh") {

				increaseTextUIScore ("LeftTextUI");
